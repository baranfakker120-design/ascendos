...
describe('Phase 8 — org branding resolution', () => {
  it('prefers branding.display_name over organizations.name', () => {
    expect(
      resolveOrgDisplayName('Chogan', { display_name: 'Team Seyda' })
    ).toBe('Team Seyda');
  ...
  it('onboarding URL comes from tools only', () => {
    const toolsA = [
      { key: 'waytomoon', url: 'https://waytomoon.netlify.app', is_active: true },
    ];
  ...
  it('rejects foreign Org-1 markers for Org B active context', () => {
    ...
    expect(
      assertNoForeignOrgFallback(orgB, 'https://teamseydaguide.netlify.app', [
        'teamseydaguide',
      ])
    ).toBe(false);
  ...
  it('parses branding JSON safely', () => {
    expect(parseOrgBranding({ display_name: 'X', guideUrl: 'https://g' }).display_name).toBe(
      'X'
    );
