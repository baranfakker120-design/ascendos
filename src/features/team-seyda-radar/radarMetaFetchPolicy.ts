 ...
 * Mirrored in supabase/functions/_shared/radar/metaFetchPolicy.ts (keep in sync).
 *
 ...
 */
export function decideMetaRetry(kind: MetaErrorKind, attemptIndex: number): MetaRetryDecision {
  ...
  if (kind === 'meta_server_error' || kind === 'meta_timeout' || kind === 'meta_network_error') {
